...
- [SociaPulse](./Projects/16-sociapulse.md) - Unified social media management SaaS across Facebook Pages/Instagram/Threads/YouTube/X, publish-first (Laravel 12 + PHP 8.4 + Inertia + Vue 3 + TS + Bootstrap/AdminLTE + PostgreSQL + Redis + Horizon) - `Desktop/Codex Lure/project/SociaPulse` - **own SaaS** - **[LEARN]** (PostgreSQL) - **PRODUCTION READY (code side) 2026-09-12** - all 7 phases verified, hardened, CSP + reply lifecycle + AC-22 restore drill run and passed; 361 tests / 1305 assertions, CS verify 100/100, security review found no HIGH/MEDIUM - repo `github.com/wafazz/SociaPulse` (private) - blocked only on §37 external prerequisites (Meta/Google verification, object storage, OQ-08 YouTube retention) + two open decisions (error tracker, backup schedule) - separate product from [12-social-listening](./Projects/12-social-listening.md) and [15-listening-console](./Projects/15-listening-console.md)
- [WebAppsBI](./Projects/17-webappsbi.md) - Custom Power BI-style Business Intelligence dashboard: Excel upload -> sheet/header detection -> reusable column mapping -> queued chunked import -> normalised financial fact table -> registry-driven analytics -> Gridstack dashboard builder (Laravel 12.69 + PHP 8.4 + REST + Vue 3.5 + TS + Bootstrap 5.3 + AdminLTE 4.9 + ECharts 6 + Gridstack 13 + PostgreSQL 16 + Redis 8 + Laravel Excel 4) - `Desktop/Codex Lure/project/WebAppsBI` - **client project, fixed fee** - **[LEARN]** (ECharts, Gridstack, Laravel Excel 4.x, Sanctum SPA) - planning approved 2026-09-13, 201 requirements / 18 phases / 37 decisions - **PH-01..PH-09 COMPLETE** (120/201 requirements, 29 tables, 51 API routes, 30 Vue SFCs, 369 PHP + 32 Vitest tests green, CS verify 100/100); Excel→import→activate→analytics→**dashboard** working end to end on real data; next PH-10 global dynamic filters with URL sync
- [LokalRevieW (LR)](./Projects/18-lokalreview.md) - Malaysia-first social-first local discovery + review network; consumers free, businesses freemium (proposed Laravel + Inertia + Vue 3 + TS + Bootstrap 5 + PostgreSQL/PostGIS + Redis + Horizon) - `Desktop/Codex Lure/project/LokalRevieW` - **own product** - registered 2026-09-14; **PLANNING ONLY** — Planning.md + 7 decision gates + verdict; no code until Fakrul approves
- [WhatsApp Business Automation SaaS](./Projects/19-whatsapp-automation-saas.md) - Multi-tenant WhatsApp inbox, campaigns, scheduler, automation, visual canvas chatbot builder + AI node on the official Meta Cloud API only (proposed Laravel 12 + PHP 8.4 + Vue 3 + TS + Bootstrap 5 + AdminLTE + MySQL 8 + Redis/Horizon + Reverb) - `Desktop/Codex Lure/project/SaaS-Whatsapp-Complete` - **own SaaS** - **[LEARN]** - registered 2026-09-16; Planning approved; **PH-00..PH-04 built and committed 2026-09-16** (16 commits on `main`, no remote); 766 PHP + 57 Vitest tests on MySQL, CS verify 100/100. Runs on MariaDB 10.4 via XAMPP:3307 — note PH-05 assumes SKIP LOCKED, which needs MariaDB 10.6+ or MySQL 8 (FU-03-05). PH-02 is code-complete against a Meta fake but has never met live Meta — staging host, a Meta app with a test WABA and Business Verification are still outstanding, by owner decision to build first and verify later. PH-03 screens S-06..S-10 and PH-04 screens S-12..S-13 are built and tested but have never been rendered in a browser (FU-03-03, FU-04-03). The four template webhook fields still need ticking in the Meta App Dashboard (FU-04-01). Installed stack: Laravel 13.32 / PHP 8.4 / Fortify / Sanctum / spatie-permission teams mode / Horizon / Reverb / libphonenumber-lite 9 / openspout 5.11 / flysystem-s3 / Vue 3.5 + TS 5.9 + AdminLTE 4.9 - separate codebase from `project/whatsapp-saas` (Evolution API, not reusable)
- [STMS](./Projects/20-stms.md) - Smart Team Management SaaS: multi-tenant work management with separate tenant workspace, platform-owner and invited-client portal contexts (Laravel 12.69 + PHP 8.4 + Inertia 2 + Vue 3.5 + TS + Tailwind 3 + shadcn-vue + MySQL 8) - `Desktop/Codex Lure/project/stms` - **own SaaS** - registered 2026-09-18; Phases 1-2 done, Phase 3 closed by **MILESTONE-001 delivered 2026-09-18** (57 tests / 179 assertions green on **both SQLite and MariaDB 10.4.28**, **CS verify 100/100**); **npm advisories 26 -> 0**, fixed by a lockfile refresh alone with `package.json` untouched. Runs on XAMPP **MariaDB 10.4.28 at port 3307**, not MySQL 8. 4 commits on `main`, local git only, no remote. **CAPACITY-001 delivered 2026-09-18** closes Phase 4 (68 tests / 309 assertions, both engines); **email verification was found decorative and is now enforced** (`User` never implemented `MustVerifyEmail`, so the `verified` middleware blocked nobody), `is_platform_owner` removed from `$fillable`; 75 tests / 343 assertions on both engines, 9 commits. **PLATFORM-001 delivered 2026-09-18** (tenant usage/health console with its own layout, suspend/reactivate, aggregates-only privacy line, no migration); 81 tests / 393 assertions on both engines, 10 commits. **REPORT-001 delivered 2026-09-18** (one service, two audiences; client redaction by allowlist; attention as named facts, no health score); 88 tests / 502 assertions on both engines, 14 commits, 5 module docs, matrix repaired to 4 Verified / 4 Built / 7 Partial / 3 Planned. Remaining work is **blocked on owner decisions**: NOTIFY-001 on OQ-3, BILL-001 on OQ-1, AI-001 on OQ-6. **SEC-001 audit trail delivered 2026-09-18** (append-only, `nullOnDelete` so a deleted tenant keeps its trail, never stores the audited secret); 98 tests / 549 assertions on both engines, 16 commits, 6 module docs. Unblocked work left: JOB-001 depth (attachments = T2 upload surface), security-event stream, production safeguards. Outstanding: no browser has ever rendered the Vue client (Chrome extension not connected), and Planning.md's traceability matrix still reads `Planned` for ~12 built requirements.
