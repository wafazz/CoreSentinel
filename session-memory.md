...
- Still open: F-15 partially (`read_json`/`print_header` still duplicated) and the dead `memorycore.conf` still ships. F-09 closed in Phase 7 (monotonic `AUD-NNNNNN`). **F-11 and F-12 closed 2026-08-14**: 12 leaked absolute paths stripped from `project-labels.json`, and the README protocol directory reconciled to the 38 documents on disk — now held by `test_the_protocol_directory_lists_every_document_on_disk`, so the count can no longer drift by hand.
